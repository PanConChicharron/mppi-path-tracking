...
int main(int argc, char** argv)
{
  ...
  const std::string video_path = "first_order_dubins_two_lane_double_park_diffusion.mp4";
  ...
  cv::namedWindow("MPPI Two-Lane Double Park (Diffusion)", cv::WINDOW_NORMAL);
  ...
  for (int k = 0; k < num_sim_steps; ++k)
  {
    ...
    mppi::viz::showCompositeFrame("MPPI Two-Lane Double Park (Diffusion)", composite, plot_layout);
    step_timing.endViz();
